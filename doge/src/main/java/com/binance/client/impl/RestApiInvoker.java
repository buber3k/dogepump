...
abstract class RestApiInvoker {
    ...
    static <T> T callSync(RestApiRequest<T> request) {
        try {
            boolean loggerDisabled = request.request.url().toString().contains("exchangeInfo") || request.request.url().toString().contains("premiumIndex");
            String str;
            if (!loggerDisabled) log.debug("Request URL " + request.request.url());
            ...
            if (!loggerDisabled) log.debug("Response =====> " + str);
